...
std::string nome_obj = "data/cubo.obj";
...
float posLuz[3] = { 0.2, 0.2, 0.5 };

float posicao[3] = { 0, 0, -0.7 };
...
std::vector<float> converterObjVector() {
    ...
    std::cout << "Numero de Desenhos: " << objeto.faces.size() * 3 << std::endl; // tamanho para glDrawArrays
    return objeto_v;
}
...
void setShaders() {
    ...
    //glBufferData(GL_ARRAY_BUFFER, vertices_v.size() * sizeof(vertices_v), &vertices_v[0], GL_STATIC_DRAW); // Para Parede
    glBufferData(GL_ARRAY_BUFFER, objeto_v.size() * sizeof(objeto_v), &objeto_v[0], GL_STATIC_DRAW); // Para OBJ
...
void display(void)
{
    ...
    //Perspectiva
    int id_projecao = glGetUniformLocation(p, "projecao");
    glUniformMatrix4fv(id_projecao,1, GL_FALSE, &matrizP[0][0]);
    ...
    //glDrawArrays(GL_QUADS, 0, 4); // Para Parede
    glDrawArrays(GL_TRIANGLES, 0, (objeto.faces.size() * 3)); // Para OBJ
...
void keyboard(unsigned char key, int x, int y)
{
    switch (key) {
        ...
        //Movimento
        case 'w': posicao[1] += 0.05; break;
        case 's': posicao[1] -= 0.05; break;
        case 'd': posicao[0] += 0.05; break;
        case 'a': posicao[0] -= 0.05; break;
        case 'q': posicao[2] += 0.05; break;
        case 'e': posicao[2] -= 0.05; break;

        // Rotação
        case 'i': rotacao[0] -= 0.1; break;
        case 'k': rotacao[0] += 0.1; break;
        case 'j': rotacao[1] -= 0.1; break;
        case 'l': rotacao[1] += 0.1; break;
        case 'u': rotacao[2] -= 0.1; break;
        case 'o': rotacao[2] += 0.1; break;

       // Escala
        case 'v': escala = escala * 2; break;
        case 'b': escala = escala * 0.5; break;

        //Iluminação
        case '1': intensidadeLuzAmbiente -= 0.05; break;
        case '2': intensidadeLuzAmbiente += 0.05; break;      
    }
...
void keyboard_special(int key, int x, int y) {
    switch (key) {
        //Iluminação movimento
        case GLUT_KEY_DOWN:  posLuz[1] -= 0.05; break;
        case GLUT_KEY_UP:    posLuz[1] += 0.05; break;
        case GLUT_KEY_RIGHT: posLuz[0] += 0.05; break;
        case GLUT_KEY_LEFT:  posLuz[0] -= 0.05; break;
    }
}

void criarPerspectivaMatriz(float matriz[4][4], float fov, float aspecto, float perto, float longe)
{
    float f = (1.0 / tan(fov)) / 2; // cotangente / 2
    matriz[0][0] = f / aspecto;
    matriz[1][1] = f;
    matriz[2][2] = (longe + perto) / (perto - longe);
    matriz[2][3] = ( 2 * longe * perto) / (perto -longe);
    //matriz[3][2] = -1; // forma correta sugerida em materiais didáticos (Talvez necessite de um offset)
    matriz[3][3] = 1;
}
...
void reshape(int width, int height)
{
    ...
    criarPerspectivaMatriz(matrizP, 60, (GLfloat)width / (GLfloat)height, 0.1f, 5000.0f);

    std::cout << "\nMatriz de projecao:" << std::endl;
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++) 
            std::cout << matrizP[i][j] << "\t";
